...
print(
    f"Baseline Accuracy: {accuracy_score(y_test, pred_base):.4f}  F1: {f1_score(y_test, pred_base, average='weighted'):.4f}"
)
```
...
for s in scoring:
    print(
        f"{s:18s} test {cv_results[f'test_{s}'].mean():.4f} +/- {cv_results[f'test_{s}'].std():.4f}"
...
grid_search = GridSearchCV(
    pipe_lgb, param_grid, cv=cv, scoring="f1_weighted", n_jobs=-1, refit=True
)
...
print(
    f"ROC-AUC: {roc_auc_score(y_test, pred_proba):.4f}   PR-AUC: {average_precision_score(y_test, pred_proba):.4f}"
)
...
importance = pd.Series(classifier.feature_importances_, index=feature_names).sort_values(
    ascending=False
)
...
predictions = X_test.assign(
    y_true=y_test.values, y_pred=pred_test, y_proba=pred_proba, run_id=RUN_ID
)
